...
class Config {
  ...
  enhanceConfig(config, configName) {
    if (configName === 'config') {
      // 读取gemini额外的模型列表
      const defaultGeminiModels = ['gemini-pro-latest', 'gemini-flash-latest', 'gemini-flash-lite-latest', 'gemini-3.1-flash-lite', 'gemini-3-flash-preview', 'gemini-3.1-flash-image-preview', 'gemini-3-pro-image-preview']
      try {
